...
from .utils import accept, generate_key
...
class ClientConnection(Connection):
    ...
    def process_subprotocol(self, headers: Headers) -> Optional[Subprotocol]:
        ...
        if subprotocols:
            ...
            if len(parsed_subprotocols) > 1:
                subprotocols = ", ".join(parsed_subprotocols)
    ...
    def parse(self) -> Generator[None, None, None]:
        response = yield from Response.parse(self.reader.readline)
        assert self.state == CONNECTING
        try:
            self.process_response(response)
        except InvalidHandshake as exception:
            self.events.append(Reject(response, exception))
            return
        else:
